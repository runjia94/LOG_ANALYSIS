...
def queryOperate(query):
    try:
        ...
        that are related to the database’s operation
...
if __name__ == '__main__':
    ...
    with open('output.txt', 'w') as f:
        ...
        for name in authorsRank:
            f.write(
                name[0] + ' --' + str(name[1]) + ' views \n')
        f.write(
            '\n3. On which days did more '
            'than 1% of requests lead to errors?\n')
        for error in errorcount:
            changetime = datetime.strptime(
                str(error[0]), '%Y-%m-%d').strftime('%B %d, %Y')
